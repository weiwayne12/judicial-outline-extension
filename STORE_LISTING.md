...
### 繁體中文
> 裁判書頁面加上「判決架構」導覽，並提供 Ctrl/Cmd+Q 智慧複製與判決剪貼簿，跨分頁保存、附字號並匯出收錄段落。

### English
> Judgment outline sidebar plus Ctrl/Cmd+Q smart copy and a side-panel clipboard that appends citations and exports saved passages.
...
✂️ **Ctrl/Cmd+Q 智慧複製**
選取任何一段判決文字後按 **Ctrl+Q**（macOS 為 **Cmd+Q**），擴充功能會自動移除分行與中日韓字元間的多餘空白，保留 ASCII 英數之間的空格，將整理後文字寫入剪貼簿並新增到「判決剪貼簿」卡片；可依設定於尾端附上「（<裁判字號>意旨參照）」。一般 **Ctrl/Cmd+C** 與 **Ctrl/Cmd+X** 均維持瀏覽器原生行為，不附字號、不新增卡片。

判決架構卡片另有「原格式全選」按鈕，只選取正文、不改寫剪貼簿內容；選取後按 **Ctrl/Cmd+C** 即走瀏覽器原生複製，保留原頁格式。

📋 **判決剪貼簿側邊欄**
每次 Ctrl/Cmd+Q 後，擴充功能會把「去分行 + 依設定附字號」的完整文字推進 Chrome 瀏覽器原生側邊欄，以卡片形式保存**當次瀏覽期間**收錄的所有判決段落。側邊欄由點擊工具列的擴充功能圖示開啟，**不限於裁判書頁面** —— 切到 Google Docs、Word Online、Obsidian Web、Notion 等任何分頁都看得到同一份卡片清單，點卡片上的「複製」鈕即可貼上，非常適合需要一次引用多段判決文字的書狀撰寫情境。
...
⚙️ **後台設定頁面**
  　• 「Ctrl/Cmd+Q 時自動附上裁判字號」開關（預設：開），亦可於判決剪貼簿側邊欄頂端即時切換，兩端共用同一設定
...
🆕 **v0.2.4 更新**
  ...
  • 新增 **Ctrl/Cmd+Q 收錄判決段落**：套用去分行與空白正規化，依設定附上字號，並存入「判決剪貼簿」卡片
  • Ctrl/Cmd+C 與 Ctrl/Cmd+X 維持瀏覽器原生複製、剪下行為，不附字號、不新增卡片
...
🆕 **v0.2.2 更新**
  • 新增支援司法院內部網路判解函釋 (`legal.law.intraj/FINT/*`)，院內使用 Chrome 亦可在判解函釋使用判決架構耳標與 Ctrl/Cmd+Q 收錄功能
...
🆕 **v0.2.0 更新**
  ...
  • Ctrl/Cmd+Q 收錄後自動把完整文字入檔為卡片，跨分頁可瀏覽、逐一再貼上，適合處理多段引文的書狀撰寫
...
🆕 **v0.1.6 更新**
  • 新增支援司法院內部網路裁判書系統 (`judgment.law.intraj/FJUD/*`)，院內使用 Chrome 亦可使用判決架構耳標與 Ctrl/Cmd+Q 收錄功能

🆕 **v0.1.5 更新**
  • 新增「擴充功能選項」設定頁
  • 新增「耳標位置左/右」可分網站設定
  • 新增「Ctrl/Cmd+Q 時自動附上裁判字號」開關
...
✂️ **Ctrl/Cmd+Q smart copy**
Select any judgment text and press **Ctrl+Q**（or **Cmd+Q** on macOS）to strip line breaks and excess CJK spacing while preserving spaces between ASCII alphanumerics, then write the cleaned text to the clipboard and save it as a card in the side panel. The citation suffix `（<case-number>意旨參照）` can be appended when enabled. Regular **Ctrl/Cmd+C** and **Ctrl/Cmd+X** keep the browser's native behavior, with no citation suffix and no card added.

📋 **Clipboard side panel**
Every Ctrl/Cmd+Q clip is saved as a card in Chrome's native side panel, accumulating **every passage saved during the current browser session**. Open the panel from the toolbar icon — it's **not limited to ruling pages**, so you can switch to Google Docs, Word Online, Obsidian Web, Notion, etc. and still see the same card list, clicking each card's "Copy" button to paste one by one. Ideal for brief-writing workflows that cite multiple passages.
...
⚙️ **Options page**
  • Toggle "append citation on Ctrl/Cmd+Q" (default: on) — also exposed at the top of the clipboard side panel as an inline switch; both endpoints share the same setting and propagate changes instantly
...
🆕 **v0.2.4 changes**
  ...
  • New **Ctrl/Cmd+Q smart-copy shortcut**: normalizes the selected text, appends the citation suffix when enabled, and adds the passage to the clipboard-card store
  • Native copy and cut behavior is preserved for Ctrl/Cmd+C and Ctrl/Cmd+X
...
🆕 **v0.2.0 changes**
  ...
  • Every Ctrl/Cmd+Q clip auto-saves as a card; cards persist across tabs and can be re-pasted one by one — ideal for multi-citation brief writing
...
**Justification（繁中）**：
擴充功能必須在司法院公開網域（FINT 法令判解系統、FJUD 裁判書系統）與司法院內部網路的裁判書及法令判解頁面上注入 DOM 元件（判決架構導覽側欄），並於使用者按 Ctrl/Cmd+Q 時攔截該次複製事件（去分行、依設定附上裁判字號、紀錄到判決剪貼簿側邊欄）。一般 Ctrl/Cmd+C 與 Ctrl/Cmd+X 維持瀏覽器原生行為。功能完全限定於這四個網域，不會存取其他網站。內網判解函釋網域（`legal.law.intraj`）限定於 `/FINT/*` 路徑；內網裁判書網域（`judgment.law.intraj`）限定於 `/FJUD/*` 路徑。行為與公開網域一致，供院內使用者安裝同一份擴充功能使用。

**Justification (English)**:
The extension needs to inject DOM elements (the outline sidebar) and, only when the user presses Ctrl/Cmd+Q, intercept that copy event to strip line breaks, optionally append a citation suffix, and log it to the clipboard side panel on ruling pages hosted at the Taiwan Judicial Yuan's public domains (FINT legal interpretations, FJUD rulings) and the Judicial Yuan's internal intranet. Regular Ctrl/Cmd+C and Ctrl/Cmd+X keep the browser's native behavior. All functionality is strictly limited to these four hosts; no other websites are accessed. The intranet legal interpretations host (`legal.law.intraj`) is scoped to `/FINT/*`; the intranet judgment host (`judgment.law.intraj`) is scoped to `/FJUD/*`. Both behave identically to the public hosts and let Judicial Yuan staff install the same extension for internal use.
