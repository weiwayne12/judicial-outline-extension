...
在瀏覽司法院法學資料檢索系統時，自動在頁面左側注入「判決架構」導覽卡片，讓長篇判決的結構一目瞭然；並提供瀏覽器側邊欄「判決剪貼簿」，以卡片形式保存當次瀏覽期間按 Ctrl/Cmd+Q 智慧複製的段落，方便跨分頁貼到 Word、Google Docs、Obsidian 等外部編輯器。
...
- **剪貼簿側邊欄內建「附上字號」即時切換**：側邊欄頂端新增「附上字號 開／關」開關，與後台設定頁的「Ctrl/Cmd+Q 時自動附上裁判字號」共用同一設定；任一端變更後即時同步至所有已開啟的分頁、設定頁與 content script，不需再點進後台。
- **Ctrl/Cmd+Q 加入判決剪貼簿**：選取文字後按 Ctrl+Q（macOS 為 Cmd+Q），套用去分行與空白正規化後寫入剪貼簿並新增卡片；可依設定附上裁判字號。
- **原格式全選正文**：判決架構卡片新增「原格式全選」按鈕，只選取正文、不改寫剪貼簿內容；選取後按 Ctrl/Cmd+C 會走瀏覽器原生複製，保留原頁格式。
- **原生複製、剪下不攔截**：Ctrl/Cmd+C 與 Ctrl/Cmd+X 均維持瀏覽器原生行為，不附字號、不新增卡片。
...
1. **判決架構側欄**（hover tab）
   ...
   - 卡片內提供「全選正文」與「原格式全選」兩顆按鈕：前者方便接續 Ctrl/Cmd+Q 智慧複製，後者方便接續 Ctrl/Cmd+C 原生複製並保留頁面格式

2. **Ctrl/Cmd+Q 智慧複製**
   - 選取文字後按 Ctrl+Q（macOS 為 Cmd+Q），將整理後文字寫入剪貼簿並新增至「判決剪貼簿」卡片
   - 自動移除分行與中日韓字元間的多餘空白，並保留 ASCII 英數之間的空格（例如 `NT 300` 不會變成 `NT300`）
   - 可於尾端附上 `（<裁判字號>意旨參照）` —— 字號從頁面「裁判字號：」欄位擷取，可在設定頁或側邊欄一鍵關閉
   - Ctrl/Cmd+C 與 Ctrl/Cmd+X 維持瀏覽器原生複製、剪下行為，不附字號、不新增卡片
   - Windows / macOS 共用同一套處理邏輯

3. **判決剪貼簿側邊欄（瀏覽器側邊欄）**
   - 每次 Ctrl/Cmd+Q 後，自動把「去分行 + 依設定附字號」的文字推進瀏覽器原生側邊欄（Chrome Side Panel），以卡片形式保存當次瀏覽期間收錄的判決段落
   ...
   - **字號附加即時切換**：側邊欄頂端提供「附上字號 開／關」切換，與設定頁的「Ctrl/Cmd+Q 時自動附上裁判字號」共享同一設定，切換後即時同步至所有分頁與設定頁，免開後台
...
| 區塊 | 設定項 | 說明 | 預設 |
|---|---|---|---|
| **複製設定** | Ctrl/Cmd+Q 時自動附上裁判字號 | 開啟時，選取文字以 Ctrl/Cmd+Q 智慧複製後尾端追加「（XX意旨參照）」；關閉時只去分行、不附字號。同一設定亦可在判決剪貼簿側邊欄頂端即時切換 | 開 |
...
- **Ctrl/Cmd+Q 時沒附上字號** → 確認你選取的文字**在判決正文區塊內**；若選到頁首按鈕或導覽就不會觸發；或檢查設定頁／側邊欄頂端的「附上字號」是否被關掉
- **想複製但不要加入剪貼簿卡片** → 改按 Ctrl/Cmd+C，完全採瀏覽器原生複製，不附字號、不新增卡片
...
| `content.js` | 核心邏輯：DOM 扁平化、階層偵測、側欄注入、Ctrl/Cmd+Q 智慧複製 handler、讀取使用者偏好、推送紀錄到剪貼簿側邊欄 |
